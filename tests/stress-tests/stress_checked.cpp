...
void stress_insert_thread(AllEnvironment<KType> *env) {
    ...
    while (!env->finished.load()) {
        ...
        if (!env->in_use[ind].test_and_set()) {
            ...
            bool res = env->table.emplace(k, v);
            bool res2 = env->table2.emplace(k, v2);
            EXPECT_NE(res, env->in_table[ind]);
            EXPECT_NE(res2, env->in_table[ind]);
            if (res) {
                EXPECT_EQ(v, env->table.find(k).value());
                EXPECT_EQ(v2, env->table2.find(k).value());
                env->vals[ind] = v;
                env->vals2[ind] = v2;
...
void delete_thread(AllEnvironment<KType> *env) {
    ...
    while (!env->finished.load()) {
        ...
        if (!env->in_use[ind].test_and_set()) {
            KType k = env->keys[ind];
            auto res = env->table.erase(k);
            auto res2 = env->table2.erase(k);
            EXPECT_EQ(res, env->in_table[ind]);
            EXPECT_EQ(res2, env->in_table[ind]);
            if (res) {
                EXPECT_FALSE(static_cast<bool>(env->table.find(k)));
                EXPECT_FALSE(static_cast<bool>(env->table2.find(k)));
        ...
        if (!env->in_use[ind].test_and_set()) {
            ...
            switch (third(gen)) {
                case 0:
                    ...
                    res = static_cast<bool>(env->table.update(k, v));
                    res2 = static_cast<bool>(env->table2.update(k, v2));
                    EXPECT_EQ(res, env->in_table[ind]);
                    EXPECT_EQ(res2, env->in_table[ind]);
                    break;
                case 1:
                    ...
                    updatefn(v);
                    updatefn2(v2);
                    res = env->table.visit(k, updatefn);
                    res2 = env->table2.visit(k, updatefn2);
                    EXPECT_EQ(res, env->in_table[ind]);
                    EXPECT_EQ(res2, env->in_table[ind]);
                    break;
                case 2:
                    ...
                    // true.
                    env->table.emplace_or_visit(k, updatefn, v);
                    env->table2.emplace_or_visit(k, updatefn2, v2);
                    res = res2 = true;
                    env->in_table[ind] = true;
                    break;
                default:
...
void find_thread(AllEnvironment<KType> *env) {
    ...
    while (!env->finished.load()) {
        ...
        if (!env->in_use[ind].test_and_set()) {
            KType k = env->keys[ind];
            auto res = env->table.find(k);
            EXPECT_EQ(env->in_table[ind], (bool) res);
            if (res) {
                EXPECT_EQ(env->vals[ind], res.value());
            }

            auto res2 = env->table2.find(k);
            EXPECT_EQ(env->in_table[ind], (bool) res2);
            if (res2) {
                EXPECT_EQ(env->vals2[ind], res2.value());
            }
...
void StressTest(AllEnvironment<KType> *env) {
    ...
    EXPECT_EQ(numfilled, env->table.make_unordered_map_view().size());
